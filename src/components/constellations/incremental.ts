...
export function initIncrementalGraph({ svgEl, width, height, state, actions }: InitIncrementalGraphArgs): IncrementalGraphState {
    ...
    const indexSel = labelLayer.selectAll<SVGTextElement, any>('text.node-index');
...
export function applyMutations(ig: IncrementalGraphState, state: any, actions: any) {
    ...
    // The same graph can feel "too tight" or "too loose" depending on node count
    // and viewport size. Since SGA graphs are often ~100-200 nodes, we treat that
    // as a "medium" graph and give it extra breathing room.
    const n = processed.nodes.length;
    const isHugeGraph = n > 350;
    const isLargeGraph = n > 150;
    const isMediumGraph = n > 80;

    const collidePad = isHugeGraph ? 4 : isLargeGraph ? 6 : isMediumGraph ? 18 : 6;
    ig.simulation.force(
        'collide',
        d3.forceCollide().radius((d: any) => ig.radiusScale(ig.nodeDegrees.get(d.id) || 1) + collidePad),
    );

    const chargeStrength = isHugeGraph ? -40 : isLargeGraph ? -120 : isMediumGraph ? -320 : -150;
    const chargeDistanceMax = isHugeGraph ? 220 : isLargeGraph ? 350 : isMediumGraph ? 1000 : 450;

    const charge = d3.forceManyBody().strength(chargeStrength).distanceMax(chargeDistanceMax);
    ig.simulation.force('charge', charge);

    const linkDistance = isHugeGraph ? 50 : isLargeGraph ? 80 : isMediumGraph ? 190 : 90;
    const linkStrength = isHugeGraph ? 0.45 : isLargeGraph ? 0.5 : isMediumGraph ? 0.45 : 0.6;
    ig.linkForce.distance(linkDistance);
    ig.linkForce.strength(linkStrength);
    ...
    // Only apply component clustering when we have multiple components.
    // For a single connected component, these forces mainly "pull everything
    // inward" and can make the layout look overly condensed.
    if (k > 1) {
        ig.simulation
            .force(
                'componentX',
    ...
    // --- Labels ----------------------------------------------------------
    // We render two separate text layers:
    //  1) node-index: center number (reading order)
    //  2) node-label: below-node label (optional; can be toggled)

    ig.indexSel = ig.labelLayer
        .selectAll('text.node-index')
        .data(processed.nodes as any, (d: any) => d.id)
        .join(
            (enter: any) => enter.append('text').attr('class', 'node-index'),
            (update: any) => update,
            (exit: any) => exit.remove(),
        )
        .text((d: any) => String(d.orderIndex ?? ''))
    ...
    state.refs.index = ig.indexSel;

    // Apply initial visibility for the new selections (especially important
    // on first render so we respect defaults like hiding node type labels).
    ...
    ig.simulation.on('tick', () => {
        ...
        // Index labels at node center with fit-to-node font sizing.
        ig.indexSel
            .attr('x', (d: any) => d.x)
            .attr('y', (d: any) => d.y)
            .each(function (this: SVGTextElement, d: any) {
                const r = ig.radiusScale(ig.nodeDegrees.get(d.id) || 1);
                const txt = String(d.orderIndex ?? '');
                if (!txt) {
                    this.textContent = '';
                    return;
                }
                // Conservative max width/height inside circle.
                const maxPx = Math.max(8, r * 1.45);
                // Start with a reasonable size; shrink if needed.
                let size = Math.max(7, Math.min(14, r));
                this.textContent = txt;
                this.style.fontSize = `${size}px`;
                // Measure and shrink.
                for (let i = 0; i < 8; i++) {
                    const box = this.getBBox();
                    if (box.width <= maxPx && box.height <= maxPx) break;
