...
function nodeTitle(d: any): string {
  // Prefer showing the original LaTeX label (\label{...}) when present.
  // In SGA exports this is stored on `node.label`.
  const lbl = (d?.label ?? '').toString().trim();
  if (lbl) return lbl;
  // Fallback to display_name ("Theorem", "Definition", ...) then id.
  return (d?.display_name || d?.id || 'Node').toString();
}
...
export function setupLegends(
  ...
  // Toggle for node type labels (below-node labels).
  const controls = nodeLegendContainer
    .append('div')
  ...
  const label = controls
    ...
    .text(state.showNodeTypeLabels ? 'Hide node type labels' : 'Show node type labels');

  controls.on('click', (event: any) => {
    event.stopPropagation();
    if (state.pinned) return;
    state.showNodeTypeLabels = !state.showNodeTypeLabels;
    label.text(state.showNodeTypeLabels ? 'Hide node type labels' : 'Show node type labels');
...
export async function renderNodeTooltip(tooltipEl: HTMLDivElement, event: any, d: any) {
  const finalPreview = formatMathToHtml(d.content_preview || d.content || 'N/A');

  tooltipEl.style.display = 'block';
  tooltipEl.innerHTML = `<h4>${nodeTitle(d)}</h4><div class="math-content">${finalPreview}</div>`;
...
export async function updateInfoPanel(
  ...
  const nodeTitleText = nodeTitle(d);
  infoTitleEl.innerHTML = `
      <span class="info-title-text">${nodeTitleText}</span>
      <button
  ...
  let infoHTML = `${proofControls}<h4>Preview</h4><div class="math-content">${formatMathToHtml(
    d.content_preview || d.content || 'N/A',
  )}</div>`;

  if (d.prerequisites_preview) {
    infoHTML += `<h4>Prerequisites</h4><div class="math-content">${formatMathToHtml(d.prerequisites_preview)}</div>`;
  }
