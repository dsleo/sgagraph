...
export default function DefinitionBankCard({ definitions }: DefinitionBankCardProps) {
    ...
    function hasMathJaxMacro(name: string): boolean {
        try {
            const win = typeof window !== 'undefined' ? (window as any) : null;
            const macros =
                (win?.MathJax?.config?.tex?.macros as Record<string, any> | undefined)
                ?? (win?.MathJax?.tex?.macros as Record<string, any> | undefined)
                ?? undefined;
    ...
    const renderDefinitionText = (raw: string): string => {
        ...
        // Prefer correct rendering when we have per-paper macros loaded into
        // MathJax (ArxiTex emits `latex_macros`), but keep a fallback for
        // legacy JSON exports that do not include macros.
        //
        // Fallback behavior: turn unknown single-letter macros like "\\F" into
        // literal letters so we render *something* instead of a MathJax error.
        return withoutControls
            .replace(/\\eps\b/g, '\\varepsilon')
            .replace(/\\([A-Z])(?![A-Za-z])/g, (_m, letter) => {
                return hasMathJaxMacro(String(letter)) ? `\\${letter}` : String(letter);
            });
    };
    ...
    const renderTerm = (raw: string): string => {
        if (!raw) return '';
        const withoutControls = raw.replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/g, '');
        // Same fallback behavior as definitions.
        return withoutControls.replace(/\\([A-Z])(?![A-Za-z])/g, (_m, letter) => {
            return hasMathJaxMacro(String(letter)) ? `\\${letter}` : String(letter);
        });
    };
